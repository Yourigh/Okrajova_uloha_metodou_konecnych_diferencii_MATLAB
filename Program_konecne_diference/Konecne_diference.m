...
set(handles.busy,'String','Èakajte prosím   Výpoèet prebieha');
guidata(hObject,handles);
drawnow();
...
if a>=b
   msg = 'a je väèšie alebo rovné b, obrá podmienky'; 
   set(handles.busy,'String',msg);
   guidata(hObject,handles);
   return;
end
if (strcmp(inputydd,'(0)'))
   msg = 'Druhá derivácia musí bu nenulová!';
   set(handles.busy,'String',msg);
   guidata(hObject,handles);
   return;
end
if presnost==0
   msg = 'Presnos musí by >0!';
   set(handles.busy,'String',msg);
   guidata(hObject,handles);
   return;
end
